...
def extract_csi(udp_payload):
    ...
    print(udp_dict)
    udp_dict["csi_matrix"]              = format_csi(udp_payload[UDP_HEADER_SIZE:],CHIPS[0]) 
    udp_dict["csi_matrix_size"]         = len(udp_dict["csi_matrix"])
    #print(udp_payload)
    #print(pkt)
    #print(udp_dict)
    #print(udp_payload[4:10].hex())
...
if __name__ == "__main__":
    ...
    for length, t, pkt in p:
        ...
        csi_array = extract_csi(udp_payload)
        entry = plot.CsiEntry().csi = csi_array
        csi_entrys.append(entry)


    for entry in csi_entrys:
